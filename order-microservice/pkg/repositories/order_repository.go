...
//
//	if OrderRepository.Database.Error != nil && !errors.Is(OrderRepository.Database.Error, gorm.ErrRecordNotFound) {
//		return nil, OrderRepository.Database.Error
//	}
//
...
func (OrderRepository OrderRepository) FindBySeller(sellerID uint, offset *int, limit *int) (*[]models.Order, error) {
	var orders []models.Order
	OrderRepository.Database = OrderRepository.Database.Preload("OrderItems", "seller_id = ?", sellerID)
...
func (OrderRepository OrderRepository) FindOneBySellerAndByOrderId(sellerID uint, orderId uint) (*models.Order, error) {
	var order models.Order
	OrderRepository.Database = OrderRepository.Database.Preload("OrderItems", "seller_id = ?", sellerID)

	OrderRepository.Database.First(&order, orderId)
